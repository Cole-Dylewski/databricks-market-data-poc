...
BRONZE_BARS_SCHEMA = StructType(
    [
        ...
        StructField(
            "batch_id", StringType(), nullable=True
        ),  # For tracking ingestion batches
    ]
)
...
BRONZE_SYMBOLS_SCHEMA = StructType(
    [
        ...
        StructField("ingestion_timestamp", TimestampType(), nullable=False),
        StructField(
            "is_active", IntegerType(), nullable=True
...
SILVER_BARS_SCHEMA = StructType(
    [
        ...
        # Quality flags
        StructField(
            "is_valid", IntegerType(), nullable=True
...
GOLD_DAILY_OHLCV_SCHEMA = StructType(
    [
        ...
        StructField(
            "num_bars", IntegerType(), nullable=True
        ),  # Number of intraday bars aggregated
        # Calculated metrics
        StructField(
            "daily_return", DoubleType(), nullable=True
        ),  # (close - open) / open
...
GOLD_ANALYTICS_SCHEMA = StructType(
    [
        ...
        StructField(
            "sma_5", DoubleType(), nullable=True
        ),  # 5-day simple moving average
        StructField(
            "sma_20", DoubleType(), nullable=True
        ),  # 20-day simple moving average
        StructField(
            "sma_50", DoubleType(), nullable=True
        ),  # 50-day simple moving average
        StructField(
            "volatility", DoubleType(), nullable=True
