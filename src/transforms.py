...
def _add_quality_checks(df: DataFrame) -> DataFrame:
    ...
    quality_score = (
        completeness_score * 0.4 + price_checks * 0.4 + reasonableness_checks * 0.2
    )

    # Add quality columns
    df = df.withColumn("quality_score", quality_score)
    df = df.withColumn(
        "is_valid", when(col("quality_score") >= 0.8, lit(1)).otherwise(lit(0))
    )
...
def aggregate_to_daily_ohlcv(silver_df: DataFrame) -> DataFrame:
    ...
    daily_df = silver_df.groupBy(
        col("symbol"), date_trunc("day", col("timestamp")).alias("trade_date")
    ).agg(
    ...
    daily_df = daily_df.withColumn(
        "daily_return",
        when(col("open") != 0, (col("close") - col("open")) / col("open")).otherwise(
            lit(None)
        ),
    )
...
def calculate_technical_indicators(daily_df: DataFrame) -> DataFrame:
    ...
    window_20d_returns = (
        Window.partitionBy("symbol").orderBy("trade_date").rowsBetween(-19, 0)
    )
    ...
    analytics_df = analytics_df.withColumn(
        "volatility", stddev("daily_return").over(window_20d_returns)
    )
...
def get_incremental_silver_data(
    spark, silver_table: str, last_processed_timestamp: Optional[datetime] = None
) -> DataFrame:
    """
...
def get_new_dates_for_gold(
    spark, daily_ohlcv_table: str, silver_table: str
) -> DataFrame:
    """
